...
PHPLiveX.prototype.HandleParams = function(user_options) {
	...
	for (param in user_options) {
		if (this.Options[param] == undefined
				&& typeof (this.Options[param]) != "object") {
	...
	if (typeof (this.Options.target) == "string") {
		...
		if (this.Options.target_attr == "innerContent") {
			if (this.Options.target.type == "select-one"
					|| this.Options.target.type == "select-multiple")
				this.Options.target_attr = "options";
			else if (this.Options.target == "[object HTMLInputElement]"
					|| this.Options.target.type != undefined)
				this.Options.target_attr = "value";
			else
				this.Options.target_attr = "innerHTML";
		}
	}
	if (typeof (this.Options.preloader) == "string") {
		if (document.getElementById(this.Options.preloader)) {
			this.Options.preloader = document
					.getElementById(this.Options.preloader);
		} else {
			errors.push("* Preloader not found with id="
...
PHPLiveX.prototype.CreatePreloading = function() {
	if (this.Options.preloader != null) {
		if (this.Options.preloader_style == "display")
			this.Options.preloader.style.display = "block";
		this.Options.preloader.style.visibility = "visible";
	}
	this.Options.onPreload.start();
	if (this.Options.clear_content)
		eval("this.Options.target." + this.Options.target_attr + " = '';");
};
...
PHPLiveX.prototype.HandleResponse = function(response) {
	if (this.Options.content_type == "json" && response != "")
		eval("response = " + response + ";");
	else if (this.Options.content_type == "xml" && response != "")
		response = this.CreateXML(response);
	if (this.Options.content_type == "text") {
		if (this.Options.eval_scripts)
			response = PLX.EvalScripts(response);
		response = PLX.EvalStyles(response);
		var test_integer = /^[\+\-]?\d*$/;
		if (response != "" && test_integer.test(response))
			response = parseInt(response);
	}
	var go = this.Options.onFinish(response, this);
	if (this.Options.target != null && this.Options.content_type != "xml"
			&& go != false) {
			...
			for ( var i = 0; i < response.length; i++) {
				...
				if (this.Options.mode == "aw" || this.Options.mode == "rw") {
					...
					if (this.Browser == "ie")
						...
						item.add(opt, item.options[0]);
				}
			}
		} else {
			switch (this.Options.mode) {
			case "aw":
				eval("item." + this.Options.target_attr + " += response;");
				break;
			case "rw":
				eval("item." + this.Options.target_attr + " = response;");
				break;
			case "asw":
				eval("item." + this.Options.target_attr + " = response + item."
						+ this.Options.target_attr + ";");
				break;
			}
		}
	}
	if (go != false)
		this.Options.onUpdate(response, this);
...
PHPLiveX.prototype.UtilizeResponse = function(funcName, funcArgs, funcUrl) {
	if (typeof (funcName) == "object") {
		if (funcName.cls)
			funcName = funcName.cls + "::" + funcName.method;
		else if (funcName.obj)
			funcName = funcName.obj + "->" + funcName.method;
	}
	...
	if (funcArgs.length > 0) {
		if (funcName) {
			for (i = 0; i < funcArgs.length; i++)
				data += "&plxa[]=" + PLX.UTF8_Encode(funcArgs[i]);
		} else {
			for (i = 0; i < funcArgs.length; i++) {
	...
	if (this.Options.method.toUpperCase() == "POST") {
		this.XML_HTTP.setRequestHeader("Method", "POST " + funcUrl
				+ " HTTP/1.1");
		this.XML_HTTP.setRequestHeader("Content-Type",
				"application/x-www-form-urlencoded");
		this.XML_HTTP
				.setRequestHeader("Accept",
						"text/javascript, text/html, text/xml, application/xml, application/json, */*");
	}
	...
	if (asynchronous) {
		...
		(this.Options.method.toUpperCase() == "GET") ? this.XML_HTTP.send(null)
				: this.XML_HTTP.send(data);
		...
		if (response != "" && test_integer.test(response))
			response = parseInt(response);
		if (_root.Options.content_type == "json" && response != "")
			eval("response = " + response + ";");
		if (_root.Options.content_type == "xml" && response != "")
			response = _root.CreateXML(response);
		return response;
	}
};
PHPLiveX.prototype.Callback = function(funcName, funcArgs) {
	...
	if (funcArgs.length != undefined) {
		...
		if (!params.id)
			params.id = PLX.RandomString();
		realArgs[realArgs.length - 1] = params;
	} else {
		if (!funcArgs.id)
			realArgs.id = PLX.RandomString();
		params = realArgs;
	}
	...
	for (i = 0; i < funcArgs.length - 1; i++) {
		if (typeof (funcArgs[i]) == "object") {
			...
			if (String(funcArgs[i]).indexOf("+"))
				args.push(String(funcArgs[i]).replace("+",
	...
	if (this.Options.interval > 0 && !PLX.Intervals[this.Options.id]
		...
		for (i = 0; i < realArgs.length; i++)
			initialArgs[i] = realArgs[i];
		initialArgs = PLX.Json2String(initialArgs);
		if (typeof (funcName) == "object")
			funcName = PLX.Json2String(funcName);
		else if (typeof (funcName) == "string")
			funcName = "'" + funcName + "'";
		PLX.Intervals[this.Options.id] = setInterval("new PHPLiveX().Callback("
				+ funcName + ", " + initialArgs + ");", this.Options.interval);
	}
	return;
};
PHPLiveX.prototype.ExternalRequest = function(options) {
	...
	if (!options.id)
		options.id = PLX.RandomString();
	var r = this.Callback(false, options);
	if (this.Options.interval > 0 && !PLX.Intervals[this.Options.id]) {
		PLX.Intervals[this.Options.id] = setInterval(
				"new PHPLiveX().Callback(false, " + PLX.Json2String(options)
...
PHPLiveX.prototype.SubmitForm = function(form, options) {
	...
	for (i = 0; i < fields.length; i++) {
		if (fields[i].name == "" || fields[i].type == "submit"
				|| fields[i].type == "button" || fields[i].type == "reset")
			continue;
		...
		if (fields[i].type == "file") {
			...
			for (k = 0; k < lim; k++) {
				if (opts[k].selected)
					args.push(fields[i].name + "~=~" + opts[k].text);
			}
		} else {
	...
	if (file_upload) {
		...
		if (!iframe) {
			...
			var iframe = "<iframe onload=\"PLX.FormIframeLoaded(this, '"
					+ form.id
					+ "')\" id=\"plx_iframe_"
					+ form.id
					+ "\" name=\"plx_iframe_"
					+ form.id
					+ "\" style=\"border:0px;padding:0px;margin:0px;width:0px;height:0px;\"></iframe>";
			var div = document.createElement("div");
			div
					.setAttribute(
							(this.Browser == "ie") ? "cssText" : "style",
							"border:0px;padding:0px;margin:0px;width:0px;height:0px;visibility:hidden;position:absolute;");
		...
		if (this.Options.interval > 0 && !PLX.Intervals[this.Options.id]) {
			PLX.Intervals[this.Options.id] = setInterval(
					"new PHPLiveX().SubmitForm('" + form.id + "', "
							+ PLX.Json2String(options) + ");",
					this.Options.interval);
		}
		return false;
	}
...
var PLX = {
	Json2String : function(obj) {
		...
		for (key in obj) {
			...
			if (typeof (obj[key]) == "string") {
				...
				if (type == "object")
					val = "\"" + key + "\": " + PLX.Json2String(obj[key]);
				else
					val = PLX.Json2String(obj[key]);
	...
	EvalStyles : function(text) {
		...
		if (styles) {
			...
			stl.type = "text/css";
			if (this.Browser == "ie")
				stl.styleSheet.cssText = csscode;
			else
				stl.innerHTML = csscode;
	...
	LoadJS : function(path, options) {
		...
		var params = PLX.JoinObjects(options, {
			...
			onFinish : function(response) {
				PLX.EvalScripts("<script type='text/javascript'>" + response
						+ "</script>");
				onFinish(response);
			}
		});
	...
	LoadCSS : function(path, options) {
		...
		var params = PLX.JoinObjects(options, {
			url : path,
			onFinish : function(response) {
				PLX.EvalStyles("<style type='text/css'>" + response
						+ "</style>");
				onFinish(response);
			}
		});
	...
	Intervals : {},
	Stop : function(id) {
		if (PLX.Intervals[id] != undefined)
			clearInterval(PLX.Intervals[id]);
	},
	Uploads : {},
	...
	InitializeUpload : function(id) {
		...
		if ((PLX.Uploads[id].allowed_types.length > 0 && !PLX.InArray(ftype,
				PLX.Uploads[id].allowed_types))
				|| PLX.InArray(ftype, PLX.Uploads[id].disallowed_types)) {
			PLX.Uploads[id].onError(PLX.TYPE_ERROR);
			return;
		}
		document.getElementById("plx_form_" + PLX.Uploads[id].uid).submit();
		if (!PLX.Uploads[id].click_el)
			PLX.Uploads[id].el.disabled = true;
		PLX.Uploads[id].BeginUpload = function() {
			PLX.ProgressUpload(id, PLX.Uploads[id].uid,
					PLX.Uploads[id].tmp_dir, {
						...
						onFinish : function(response, root) {
							...
							if (PLX.Uploads[id].uploaded.length == 2)
								PLX.Uploads[id].uploaded.shift();
							PLX.Uploads[id].uploaded.push(response.uploaded);
							var bytes_appended = PLX.Uploads[id].uploaded[1]
									- PLX.Uploads[id].uploaded[0];
							var speed = parseFloat(bytes_appended
									/ (PLX.Uploads[id].interval / 1000));
							var progress = {
							...
							if (response.percent == 100) {
								PLX.StopUpload(id);
								progress = PLX.JoinObjects(progress, {
									completed : true,
									file_tmp_name : response.file_tmp_name,
		...
		setTimeout("PLX.Uploads['" + id + "'].BeginUpload();",
				PLX.Uploads[id].interval);
	},
	ProgressUpload : function(id, options) {
		return new PHPLiveX().Callback( {
			instance : "PLX",
			cls : "PHPLiveX",
			method : "ProgressUpload"
		}, PLX.ProgressUpload.arguments);
	},
	StopUpload : function(id) {
		PLX.Stop(PLX.Uploads[id].uid);
		if (!PLX.Uploads[id].click_el)
			PLX.Uploads[id].el.disabled = false;
	},
	AjaxifyUpload : function(elements, cfg) {
		...
		if (typeof (elements) == "string"
				|| (typeof (elements) == "object" && elements.length == undefined))
			elements = [ elements ];
		for ( var i = 0; i < elements.length; i++) {
			var el = (typeof (elements[i]) == "string") ? document
					.getElementById(elements[i]) : elements[i];
			var uid = new Date().getTime() + PLX.RandomString();
			PLX.Uploads[el.id] = PLX.JoinObjects( {
				uploaded : [ 0 ],
				el : el,
				uid : uid
			}, config);
			try {
				var frm = document
						.createElement("<form enctype=\"multipart/form-data\">");
				var iframe = document
						.createElement("<iframe name=\"plx_iframe_" + uid
								+ "\">");
			} catch (ex) {
			...
			frm.action = config.cgi_path + "?uid=" + uid + "&max_size="
					+ config.max_size + "&insensitivity="
	...
	FormIframeLoaded : function(iframe, form_id) {
		...
		PLX.FormIframes[form_id].HandleResponse(response,
				PLX.FormIframes[form_id]);
	}
};